...
public:
  ...
  : Node("map_georeference_exporter")
  {
    ...
    setStatus("waiting for the optimized LIO-SAM path and fixed RTK history");
  }
...
private:
  ...
  {
    rclcpp::Time stamp;
    Eigen::Isometry3d enu_to_target{Eigen::Isometry3d::Identity()};
  };
  ...
  {
    if (message->header.frame_id != enu_frame_ || message->child_frame_id != target_frame_) {
      RCLCPP_WARN_THROTTLE(
        get_logger(), *get_clock(), 3000,
        "Ignoring RTK odometry with frames %s -> %s; expected %s -> %s",
        message->header.frame_id.c_str(), message->child_frame_id.c_str(),
        enu_frame_.c_str(), target_frame_.c_str());
      return;
    }
    TimedRtkPose sample;
    sample.stamp = rclcpp::Time(message->header.stamp);
    try {
      sample.enu_to_target = messagePose(message->pose.pose);
    } catch (const std::exception & error) {
      RCLCPP_WARN(get_logger(), "Ignoring invalid RTK odometry: %s", error.what());
      return;
    }
    std::lock_guard<std::mutex> lock(mutex_);
  ...
  std::vector<GeoreferenceSample> synchronizedFinalSamples(
    ...
    const std::deque<TimedRtkPose> & rtk_history) const
  {
    ...
    for (const auto & stamped_pose : path.poses) {
      ...
      try {
        const Eigen::Isometry3d map_to_target = messagePose(stamped_pose.pose);
        GeoreferenceSample sample;
        sample.enu_position = rtk.enu_to_target.translation();
        sample.map_position = map_to_target.translation();
        sample.enu_yaw = poseYaw(rtk.enu_to_target);
        sample.map_yaw = poseYaw(map_to_target);
        sample.has_yaw = true;
  ...
  MapGeoreference buildGeoreference()
  {
    ...
    std::deque<TimedRtkPose> rtk_history;
    ...
    const std::vector<GeoreferenceSample> samples =
      synchronizedFinalSamples(optimized_path, rtk_history);
