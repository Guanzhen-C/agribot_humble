...
手机只拿到按用户顺序排列的目标地点和明确禁止通行的地点。RDK把目标交给Nav2 Smac，并在禁行地点附近叠加有限范围的指数避让代价；中心最高、随距离快速衰减且半径外为零，不会形成硬障碍。服务端不运行拓扑A*，也不生成路线偏好代价。服务器不能直接控制底盘。百炼 API Key 和 Neo4j 密码只保存在服务器的 `semantic.env`，不得写入 APK、网页资源或 Git。
