...
bool VIOManager::insertPointIntoVoxelMap(VisualPoint *pt_new)
{
  ...
  {
    if (static_cast<int>(iter->second->voxel_points.size()) >= max_points_per_voxel)
    {
      return false;
    }
    iter->second->voxel_points.push_back(pt_new);
    iter->second->count = static_cast<int>(iter->second->voxel_points.size());
  }
  else
  {
    VOXEL_POINTS *ot = new VOXEL_POINTS(1);
      ...
      if (normal_en)
      {
        ...
        else if (!pt->has_ref_patch_)
        {
          for (auto it = pt->obs_.begin(), ite = pt->obs_.end(); it != ite; ++it)
          {
            ...
            float *patch_temp = ref_patch_temp->patch_.get();
            ...
            for (auto itm = pt->obs_.begin(), itme = pt->obs_.end(); itm != itme; ++itm)
            {
              ...
              float *patch_cache = (*itm)->patch_.get();
...
void VIOManager::generateVisualMapPoints(cv::Mat img, vector<pointWithVar> &pg)
{
  ...
  for (int i = 0; i < length; i++)
  {
    if (grid_num[i] == TYPE_POINTCLOUD) // && (scan_value[i]>=50))
    {
      ...
      getImagePatch(img, pc, patch.get(), 0);

      VisualPoint *pt_new = new VisualPoint(pt);

      Vector3d f = cam->cam2world(pc);
      Feature *ftr_new = new Feature(pt_new, std::move(patch), pc, f, new_frame_->T_f_w_, 0);
      ftr_new->img_ = img;
...
void VIOManager::updateVisualMapPoints(cv::Mat img)
{
  ...
  for (int i = 0; i < total_points; i++)
  {
    ...
    if (add_flag)
    {
      update_num += 1;
      update_flag[i] = 1;
      auto patch_temp = std::make_unique<float[]>(patch_size_total);
      getImagePatch(img, pc, patch_temp.get(), 0);
      Vector3d f = cam->cam2world(pc);
      Feature *ftr_new = new Feature(pt, std::move(patch_temp), pc, f, new_frame_->T_f_w_, visual_submap->search_levels[i]);
      ftr_new->img_ = img;
      ftr_new->id_ = new_frame_->id_;
      ftr_new->inv_expo_time_ = state->inv_expo_time;
