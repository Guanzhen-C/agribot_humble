...
struct Feature
{
  ...
  std::unique_ptr<float[]> patch_; //!< Image patch data owned by this feature.
  ...
  Feature(VisualPoint *_point, std::unique_ptr<float[]> _patch, const Vector2d &_px, const Vector3d &_f, const SE3<double> &_T_f_w, int _level)
      : type_(CORNER), px_(_px), f_(_f), T_f_w_(_T_f_w), mean_(0), score_(0), level_(_level), patch_(std::move(_patch)), point_(_point)
  {
  }
