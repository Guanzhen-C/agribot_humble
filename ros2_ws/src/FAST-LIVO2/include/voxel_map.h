...
public:
  ...
  bool mapSliding();
  size_t clearMemOutOfMap(const int& x_max,const int& x_min,const int& y_max,const int& y_min,const int& z_max,const int& z_min );

private:
  bool isPointWithinLocalMap(const V3D &point) const;
