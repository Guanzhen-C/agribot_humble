...
namespace gazebo
{
class AgribotAckermannGazeboSceneSync : public WorldPlugin
{
public:
  void Load(physics::WorldPtr world, sdf::ElementPtr) override
  {
    ...
    update_connection_ = event::Events::ConnectWorldUpdateBegin(
      std::bind(
        &AgribotAckermannGazeboSceneSync::OnUpdate,
        this,
        std::placeholders::_1));
  }
...
private:
  void OnUpdate(const common::UpdateInfo & info)
  {
    ...
    for (const auto & model : world_->Models()) {
      // Static geometry only needs an initial render-scene synchronization.
      // Repacking hundreds of unchanged orchard objects every 5 ms consumes a
      // measurable share of the Jetson CPU without changing camera/LiDAR data.
      if (info.simTime <= common::Time(1, 0) || !model->IsStatic()) {
        models.push(model);
      }
    }
...
GZ_REGISTER_WORLD_PLUGIN(AgribotAckermannGazeboSceneSync)
}  // namespace gazebo
