...
*Remarks:*
- Livox sensors require **livox_ros_driver2**, which is included in this workspace.
- Build the workspace with `colcon` and source `install/setup.bash` before starting FAST-LIO.
...
Files can be downloaded from [Google Drive](https://drive.google.com/drive/folders/1CGYEJ9-wWjr8INyan6q1BZz_5VtGB-fP?usp=sharing). Convert legacy datasets to rosbag2 storage before playback.
...
We provide [dataset recordings](https://drive.google.com/drive/folders/1VBK5idI1oyW0GC_I_Hxh63aqam3nocNK?usp=sharing) and [a conversion script](https://drive.google.com/file/d/1leh7DxbHx29DyS1NJkvEfeNJoccxH7XM/view). Convert the generated recording to rosbag2 storage before playback.
    
Run:
```bash
ros2 launch fast_lio mapping.launch.py config_file:=velodyne.yaml
ros2 bag play YOUR_DOWNLOADED_ROSBAG2_DIRECTORY
```
