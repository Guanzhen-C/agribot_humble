...
public:
  ...
  /// Physics objects used to bypass Gazebo transport-based IMU updates
  gazebo::physics::WorldPtr world_;
  gazebo::physics::LinkPtr link_;
  /// Event triggered when simulation advances
  gazebo::event::ConnectionPtr world_update_event_;
  /// Sampling state
  gazebo::common::Time last_update_time_;
  ignition::math::Vector3d last_linear_velocity_;
  ignition::math::Quaterniond orientation_reference_;
  bool initialized_{false};
  bool initial_orientation_as_reference_{false};
...
void GazeboRosImuSensor::Load(gazebo::sensors::SensorPtr _sensor, sdf::ElementPtr _sdf)
{
  ...
  impl_->initial_orientation_as_reference_ = initial_orientation_as_reference;

  impl_->world_ = gazebo::physics::get_world(impl_->sensor_->WorldName());
  if (!impl_->world_) {
    RCLCPP_ERROR(impl_->ros_node_->get_logger(), "Unable to find the sensor world. Exiting.");
    return;
  }
  impl_->link_ = boost::dynamic_pointer_cast<gazebo::physics::Link>(
    impl_->world_->EntityByName(impl_->sensor_->ParentName()));
  if (!impl_->link_) {
    RCLCPP_ERROR(
      impl_->ros_node_->get_logger(), "Unable to find parent link [%s]. Exiting.",
      impl_->sensor_->ParentName().c_str());
    return;
  }
  ...
  impl_->world_update_event_ = gazebo::event::Events::ConnectWorldUpdateBegin(
    std::bind(&GazeboRosImuSensorPrivate::OnUpdate, impl_.get(), std::placeholders::_1));
  impl_->sensor_->SetActive(true);
}

void GazeboRosImuSensorPrivate::OnUpdate(const gazebo::common::UpdateInfo & _info)
{
  const double update_rate = sensor_->UpdateRate();
  if (initialized_ && update_rate > 0.0 &&
    (_info.simTime - last_update_time_).Double() < 1.0 / update_rate)
  {
    return;
  }

  const auto sensor_pose = link_->WorldPose() * sensor_->Pose();
  const auto linear_velocity = link_->WorldLinearVel(
    sensor_->Pose().Pos(), link_->WorldPose().Rot());

  if (!initialized_) {
    last_update_time_ = _info.simTime;
    last_linear_velocity_ = linear_velocity;
    orientation_reference_ = initial_orientation_as_reference_ ?
      sensor_pose.Rot() : ignition::math::Quaterniond::Identity;
    initialized_ = true;
    return;
  }

  const double dt = (_info.simTime - last_update_time_).Double();
  if (dt <= 0.0) {
    return;
  }

  auto angular_velocity = sensor_pose.Rot().RotateVectorReverse(link_->WorldAngularVel());
  auto linear_acceleration = sensor_pose.Rot().RotateVectorReverse(
    (linear_velocity - last_linear_velocity_) / dt - world_->Gravity());
  last_update_time_ = _info.simTime;
  last_linear_velocity_ = linear_velocity;

  using SNT = gazebo::sensors::SensorNoiseType;
  angular_velocity.X(
    sensor_->Noise(SNT::IMU_ANGVEL_X_NOISE_RADIANS_PER_S)->Apply(
      angular_velocity.X(), dt));
  angular_velocity.Y(
    sensor_->Noise(SNT::IMU_ANGVEL_Y_NOISE_RADIANS_PER_S)->Apply(
      angular_velocity.Y(), dt));
  angular_velocity.Z(
    sensor_->Noise(SNT::IMU_ANGVEL_Z_NOISE_RADIANS_PER_S)->Apply(
      angular_velocity.Z(), dt));
  linear_acceleration.X(
    sensor_->Noise(SNT::IMU_LINACC_X_NOISE_METERS_PER_S_SQR)->Apply(
      linear_acceleration.X(), dt));
  linear_acceleration.Y(
    sensor_->Noise(SNT::IMU_LINACC_Y_NOISE_METERS_PER_S_SQR)->Apply(
      linear_acceleration.Y(), dt));
  linear_acceleration.Z(
    sensor_->Noise(SNT::IMU_LINACC_Z_NOISE_METERS_PER_S_SQR)->Apply(
      linear_acceleration.Z(), dt));
  ...
  msg_->header.stamp = gazebo_ros::Convert<builtin_interfaces::msg::Time>(_info.simTime);
  msg_->orientation =
    gazebo_ros::Convert<geometry_msgs::msg::Quaternion>(
    orientation_reference_.Inverse() * sensor_pose.Rot());
  msg_->angular_velocity =
    gazebo_ros::Convert<geometry_msgs::msg::Vector3>(angular_velocity);
  msg_->linear_acceleration =
    gazebo_ros::Convert<geometry_msgs::msg::Vector3>(linear_acceleration);
#ifdef IGN_PROFILER_ENABLE
  IGN_PROFILE_END();
