...
void GazeboRosFactoryPrivate::OnWorldCreated(const std::string &)
{
  ...
  gazebo_msgs::srv::SpawnEntity::Response::SharedPtr res)
{
  ...
  // Queue model creation, or load a light directly through the Gazebo 11 API.
  if (isLight) {
    auto world_sdf = world_->SDF();
    auto light_sdf = entity_elem->Clone();
    light_sdf->SetParent(world_sdf);
    world_sdf->InsertElement(light_sdf);

    auto world_root = world_->BaseByName(world_->Name());
    if (!world_root || !world_->LoadLight(light_sdf, world_root)) {
      res->success = false;
      res->status_message = "Failed to load light [" + req->name + "]";
      return;
    }
  } else {
...
void GazeboRosFactoryPrivate::DeleteEntity(
  ...
  gazebo_msgs::srv::DeleteEntity::Response::SharedPtr res)
{
  if (!world_->ModelByName(req->name) && !world_->LightByName(req->name)) {
    res->success = false;
    res->status_message = "Entity [" + req->name + "] does not exist";
    return;
  }

  // World::RemoveModel handles both top-level models and lights in Gazebo 11.
  world_->RemoveModel(req->name);
