...
int main(int argc, char** argv) {
    ...
    try {
        if (lidar_type == "CX") {
            ...
            LS_ERROR << "This workspace only builds the CX LSLIDAR driver. "
                     << "Configured lidar_type='" << lidar_type << "'." << LS_END;
            throw std::invalid_argument("Unsupported lidar type for CX-only build");
        }
